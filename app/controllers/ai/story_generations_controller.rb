...
module AI
  class StoryGenerationsController < ApplicationController
    ...
    before_action :set_story_generation, only: [ :show, :retry ]
    ...
    def retry
      if @story_generation.status_failed?
        # Retry the entire generation
        AnalyzeDrawingsJob.perform_later(@story_generation.id)
        @story_generation.update!(status: :pending, error_message: nil)

        redirect_to ai_child_profile_book_story_generation_path(@child_profile, @book, @story_generation),
                    notice: "Retrying story generation..."
      else
        redirect_to ai_child_profile_book_story_generation_path(@child_profile, @book, @story_generation),
                    alert: "Cannot retry a generation that is not failed."
      end
    end
